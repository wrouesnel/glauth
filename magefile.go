...
const (
	// directory where the application actually resides
	constPkgDir = "v2"
)
...
var pkgDir = normalizePath(path.Join(curDir, constPkgDir))
var cmdDir = normalizePath(path.Join(pkgDir, constCmdDir))
...
func init() {
	...
	goSrc = func() []string {
		results := new([]string)
		err := filepath.Walk(pkgDir, func(relpath string, info os.FileInfo, _ error) error {
	...
	goPkgs = func() []string {
		results := []string{}
		panicOnError(os.Chdir(pkgDir))
		defer func() { panicOnError(os.Chdir(curDir)) }()
...
func makeBuilder(cmd string, platform Platform) func() error {
	f := func() error {
		cmdSrc := fmt.Sprintf("./%s/%s", must(filepath.Rel(pkgDir, cmdDir)), cmd)
		...
		return sh.RunWith(map[string]string{"CGO_ENABLED": "1", "GOOS": platform.OS, "GOARCH": platform.Arch},
			"go", "build", "-a", "-ldflags", fmt.Sprintf("-buildid='' -X main.Version=%s", version),
...
//nolint:unparam
func Debug() error {
	fmt.Println("Package Dir:", pkgDir)
